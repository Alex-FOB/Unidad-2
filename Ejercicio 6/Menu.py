...
class Interfaz:
    ...
    def verificacion(self): #permite verificar el contenido de la lista
        if(self.__manejador.verificarLista() == True):
            print('La lista se creo correctamente')
        else:
            print('ERROR: la lista no se creó')
    def entrada(self): #obtiene la posición del viajero en la lista
        ...
        if(self.__pos != False):
            band = True
        return band
    def punto1(self): #lista a el/los vaijeros con la mayor cantidad de millas
        pos = self.__manejador.punto1()
        lista = self.__manejador.punto1Verf(pos)
        for viajero in lista:
            print(viajero)
        input()
    def punto2(self): #acumula millas
